...
# Adjust these probabilities to control the amount of messy data
MESSY_DATA_CONFIG = {
    # Weather Dataset Issues
    'weather': {
        'duplicate_id': 0.03,           
        'missing_id': 0.01,             
        'invalid_date_format': 0.08,    
        'missing_date': 0.02,           
        'missing_city': 0.01,           
        'season_inconsistency': 0.03,   
        'temp_outliers': 0.01,          
        'missing_temp': 0.02,           
        'humidity_outliers': 0.01,      
        'missing_humidity': 0.01,       
        'extreme_rain': 0.03,           
        'missing_rain': 0.02,           
        'wind_outliers': 0.01,          
        'missing_wind': 0.03,           
        'visibility_strings': 0.01,     
        'extreme_visibility': 0.03,     
        'missing_visibility': 0.03,     
        'missing_condition': 0.03,      
        'missing_pressure': 0.02,       
        'duplicate_rows': 0.01,         
    },
    # Traffic Dataset Issues
    'traffic': {
        'duplicate_id': 0.02,           
        'missing_id': 0.01,             
        'invalid_date_format': 0.08,    
        ...
        'missing_city': 0.02,           
        'missing_area': 0.93,           
        'extreme_vehicles': 0.02,       
        'missing_vehicles': 0.01,       
        'negative_speed': 0.01,         
        'missing_speed': 0.02,          
        'extreme_accidents': 0.01,      
        'missing_accidents': 0.03,      
        'invalid_congestion': 0.05,     
        'missing_congestion': 0.03,     
        'missing_road_condition': 0.03, 
        'extreme_visibility': 0.05,     
        'missing_visibility': 0.05,     
        'duplicate_rows': 0.01,         
    }
}
client = Minio(
...
def generate_base_timestamps(n_records=5000):
    ...
    # Generate 95% matching timestamps
    matching_count = int(n_records * 0.95)
    ...
    # Add 5% random timestamps for variety
...
def generate_synthetic_weather_dataset(n_records=5000, base_timestamps=None):
    ...
    for i in range(n_records):
        # ID with some duplicates
        if i > 0 and random.random() < MESSY_DATA_CONFIG['weather']['duplicate_id']:
            data['weather_id'].append(data['weather_id'][-1])  # Duplicate last ID
        elif random.random() < MESSY_DATA_CONFIG['weather']['missing_id']:
        ...
        invalid_threshold = MESSY_DATA_CONFIG['weather']['invalid_date_format']
        missing_threshold = MESSY_DATA_CONFIG['weather']['missing_date']
        
        if format_choice < (1 - invalid_threshold - missing_threshold):  # Correct format
            date_str = current_date.strftime("%Y-%m-%d %H:%M")
        elif format_choice < (1 - missing_threshold):  # Alternative/invalid formats
            alt_choice = random.random()
            if alt_choice < 0.33:
                date_str = current_date.strftime("%d/%m/%Y %I%p")
            elif alt_choice < 0.67:
        ...
        if random.random() < (1 - MESSY_DATA_CONFIG['weather']['missing_city']):
        ...
        if date_str and format_choice < 0.80:  # Only for valid dates
            ...
            # Introduce some season inconsistencies
            if random.random() < MESSY_DATA_CONFIG['weather']['season_inconsistency']:
        ...
        if season and season in season_ranges:
            ...
            if random.random() < (1 - MESSY_DATA_CONFIG['weather']['temp_outliers']):
        ...
        if random.random() < MESSY_DATA_CONFIG['weather']['missing_temp']:
            temperature = None
        
        data['temperature_c'].append(temperature)
        
        # Humidity (20-100%, with outliers)
        if random.random() < (1 - MESSY_DATA_CONFIG['weather']['humidity_outliers']):
            humidity = random.randint(20, 100)
        else:
            humidity = random.choice([-10, 150, 200, -5])  # Outliers
        
        # Missing humidity
        if random.random() < MESSY_DATA_CONFIG['weather']['missing_humidity']:
            humidity = None
        
        data['humidity'].append(humidity)
        
        # Rainfall (mm) - mostly 0-50, some extremes
        if random.random() < (1 - MESSY_DATA_CONFIG['weather']['extreme_rain']):
        ...
        # Missing rain values
        if random.random() < MESSY_DATA_CONFIG['weather']['missing_rain']:
            rain = None
        
        data['rain_mm'].append(rain)
        
        # Wind speed (km/h) with outliers
        if random.random() < (1 - MESSY_DATA_CONFIG['weather']['wind_outliers']):
        ...
        # Missing wind speed
        if random.random() < MESSY_DATA_CONFIG['weather']['missing_wind']:
            wind_speed = None
        
        data['wind_speed_kmh'].append(wind_speed)
        
        # Visibility (meters) with issues
        visibility_choice = random.random()
        missing_vis = MESSY_DATA_CONFIG['weather']['missing_visibility']
        extreme_vis = MESSY_DATA_CONFIG['weather']['extreme_visibility']
        string_vis = MESSY_DATA_CONFIG['weather']['visibility_strings']
        normal_vis = 1 - missing_vis - extreme_vis - string_vis
        
        if visibility_choice < normal_vis:  # Normal range
            visibility = random.randint(50, 10000)
        elif visibility_choice < (normal_vis + extreme_vis):  # Extreme values
            visibility = random.randint(20000, 50000)
        elif visibility_choice < (normal_vis + extreme_vis + string_vis):  # Non-numeric strings
        ...
        if random.random() < (1 - MESSY_DATA_CONFIG['weather']['missing_condition']):
        ...
        # Air pressure (hPa)
        if random.random() < (1 - MESSY_DATA_CONFIG['weather']['missing_pressure']):
    ...
    duplicate_indices = random.sample(range(100, n_records), int(n_records * MESSY_DATA_CONFIG['weather']['duplicate_rows']))
...
def generate_synthetic_traffic_dataset(n_records=5000, base_timestamps=None):
    ...
    # London area/district names
    london_areas = ['Camden', 'Chelsea', 'Islington', 'Southwark', 'Kensington', 
                    'Westminster', 'Greenwich', 'Hackney', 'Tower Hamlets', 'Lambeth']
    ...
    for i in range(n_records):
        ...
        if i > 0 and random.random() < MESSY_DATA_CONFIG['traffic']['duplicate_id']:
            data['traffic_id'].append(data['traffic_id'][-1])  # Duplicate
        elif random.random() < MESSY_DATA_CONFIG['traffic']['missing_id']:
        ...
        format_choice = random.random()
        invalid_threshold = MESSY_DATA_CONFIG['traffic']['invalid_date_format']
        missing_threshold = MESSY_DATA_CONFIG['traffic']['missing_date']
        ...
        # City (mostly London, some missing)
        if random.random() < (1 - MESSY_DATA_CONFIG['traffic']['missing_city']):
            data['city'].append('London')
        else:
            data['city'].append(None)
        
        # Area - mostly missing
        if random.random() < MESSY_DATA_CONFIG['traffic']['missing_area']:
            area = None
        else:
            # Generate actual London district names when present
            area = random.choice(london_areas)
        
        data['area'].append(area)
        
        # Vehicle count with outliers
        if random.random() < (1 - MESSY_DATA_CONFIG['traffic']['extreme_vehicles']):
        ...
        if random.random() < MESSY_DATA_CONFIG['traffic']['missing_vehicles']:
            vehicles = None
        ...
        if vehicles is not None and vehicles > 0:
            ...
            # Chance of negative speed
            if random.random() < MESSY_DATA_CONFIG['traffic']['negative_speed']:
        ...
        # Missing speeds
        if random.random() < MESSY_DATA_CONFIG['traffic']['missing_speed']:
            speed = None
        
        data['avg_speed_kmh'].append(speed)
        
        # Accident count (rare events with some extremes)
        if random.random() < (1 - MESSY_DATA_CONFIG['traffic']['extreme_accidents']):
        ...
        # Missing accident counts
        if random.random() < MESSY_DATA_CONFIG['traffic']['missing_accidents']:
            accidents = None
        ...
        if vehicles is not None and speed is not None and speed >= 0:
            ...
            # Incorrect congestion categories
            if random.random() < MESSY_DATA_CONFIG['traffic']['invalid_congestion']:
                congestion = random.choice(['Very High', 'Low-Medium', 'Extreme', ''])
        else:
            congestion = random.choice(congestion_levels)
        
        # Missing congestion
        if random.random() < MESSY_DATA_CONFIG['traffic']['missing_congestion']:
            congestion = None
        
        data['congestion_level'].append(congestion)
        
        # Road condition
        if random.random() < (1 - MESSY_DATA_CONFIG['traffic']['missing_road_condition']):
        ...
        visibility_choice = random.random()
        missing_vis = MESSY_DATA_CONFIG['traffic']['missing_visibility']
        extreme_vis = MESSY_DATA_CONFIG['traffic']['extreme_visibility']
        
        if visibility_choice < (1 - missing_vis - extreme_vis):
            visibility = random.randint(50, 10000)
        elif visibility_choice < (1 - missing_vis):  # Extreme
            visibility = random.randint(15000, 30000)
        else:
            visibility = None
    ...
    # Add duplicate rows
    duplicate_indices = random.sample(range(100, n_records), int(n_records * MESSY_DATA_CONFIG['traffic']['duplicate_rows']))
    for idx in duplicate_indices:
        if idx < len(df):
            duplicate_row = df.iloc[idx].copy()
            df = pd.concat([df, pd.DataFrame([duplicate_row])], ignore_index=True)
...
if __name__ == "__main__":
    ...
    traffic_buffer = io.StringIO() 

    weather_df.to_csv(weather_buffer, index=False)
    traffic_df.to_csv(traffic_buffer, index=False)
    weather_bytes = weather_buffer.getvalue().encode('utf-8')
    traffic_bytes = traffic_buffer.getvalue().encode('utf-8')

    # upload raw files to MinIO bronze bucket
    client.put_object("bronze", "synthetic_weather_data.csv", io.BytesIO(weather_bytes), len(weather_bytes), content_type="text/csv")
    client.put_object("bronze", "synthetic_traffic_data.csv", io.BytesIO(traffic_bytes), len(traffic_bytes), content_type="text/csv")
